...
class LocalFileSizeFilter implements Filterable, PathFilterable
{
    const MB = 'M';
    const KB = 'K';
    const GB = 'G';
    ...
    public function applyPathFilters(array $headers) {
        ...
        if(!array_key_exists('size', $headers)) {
            throw new \Exception('Invalid headers for LocalFile size applyPathFilters');
        }
        $fileSize = $headers['size'];
        
        if($fileSize > $this->size) {
            throw new FileSizeException('File exceeds size limit passed by '.$this->size. 'kb confgured current file is '.$fileSize. ' kb');
        }
    }
